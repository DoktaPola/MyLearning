...
private:
    ...
    void vanish(Node *node);

    void vanishSecond(Node *node);
};
...
void Tree::vanish(Node *node) {
    if (!node) {
        return;
    } else {
        auto new_left = node->left;
        auto new_right = node->right;
        delete node;
...
void Tree::vanishSecond(Node *node) {
    if (!node){
        return;
    } else{
        auto new_left = node->left;
        auto new_right = node->right;
        vanishSecond(new_left);
        vanishSecond(new_right);
        delete node;
    }
}
